// components/admin/AddEditForm.tsx
"use client";
...
const EditForm = ({ product, closeModal }: any) => {
  ...
  const onSubmit = async (data: ProductValues) => {
    ...
    try {
      ...
      // refresh and navigate after update
      router.refresh();
      closeModal(true);
      router.push("/admin/products");
    } catch (err: any) {
      console.error("Failed to update product", err);
      toast.error(err?.message ?? "Failed to update product. Try again.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div
      ...
      className="addForm relative h-[70vh] md:h-[90vh] z-10 w-full max-w-xl bg-white rounded-lg px-6 py-6 shadow-lg overflow-auto"
    >
        ...
        >
          ...
          <footer className="mt-4 flex items-center justify-end gap-2">
            <ConfirmDialog
              ...
              cancelLabel="Cancel"
              destructive={false}
              // IMPORTANT: form.handleSubmit(onSubmit) returns a function — pass it directly
              ...
              trigger={
                ...
                >
                  {submitting || isPending ? "Editing..." : "Edit product"}
                </Button>
              }
            />
          </footer>
